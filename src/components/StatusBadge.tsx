...
const config: Record<string, { label: string; dot: string }> = {
  ...
  partially_fulfilled: { label: 'Partially Fulfilled',  dot: 'bg-orange-400' },
  fulfilled:           { label: 'Fulfilled',            dot: 'bg-green-400' },
  ...
  completed:           { label: 'Completed',            dot: 'bg-black' },
}

export function StatusBadge({ status }: { status: string }) {
  const c = config[status] ?? { label: status, dot: 'bg-gray-300' }
  return (
