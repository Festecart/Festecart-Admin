...
interface CourierVendor {
  ...
  tracking_number_mandatory: boolean
  created_at: string
}
...
interface VendorModalProps {
  vendor: CourierVendor | null   // null = add mode
  onClose: () => void
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h3 className="font-semibold text-gray-900 text-base">
            {vendor ? 'Edit Courier Vendor' : 'Add Courier Vendor'}
          </h3>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Courier Vendor Name"
          ...
          {/* Tracking Number placeholder tag */}
          <div>
            <button
              type="button"
              onClick={() => setTrackingUrl(u => u + '{tracking_number}')}
              className="px-3 py-1.5 text-xs font-medium border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-700"
            >
              Tracking Number
            </button>
            <p className="text-xs text-gray-400 mt-1">
              Click to insert <code className="bg-gray-100 px-1 rounded">{'{'+'tracking_number'+'}'}</code> placeholder into the URL
            </p>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Tracking Number to be mandatory?
            </label>
            <div className="flex gap-6">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="radio" name="mandatory" checked={mandatory}
        ...
        <div className="flex gap-3 justify-end px-6 py-4 border-t border-gray-100">
          ...
          <button
            onClick={() => onSave({ name, tracking_url: trackingUrl || null, tracking_number_mandatory: mandatory })}
...
export default function CourierVendors() {
  ...
  const handleSave = async (payload: Omit<CourierVendor, 'id' | 'created_at'>) => {
    ...
    try {
      if (editingVendor) {
        const { error } = await supabase.from('courier_vendors')
          .update(payload).eq('id', editingVendor.id)
        if (error) throw error
      } else {
        const { error } = await supabase.from('courier_vendors').insert(payload)
        if (error) throw error
      }
      qc.invalidateQueries({ queryKey: ['courier-vendors'] })
      setShowModal(false)
    } catch (e) {
      setModalError((e as { message?: string })?.message ?? 'Save failed')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    await supabase.from('courier_vendors').delete().eq('id', id)
    qc.invalidateQueries({ queryKey: ['courier-vendors'] })
                      ...
                      <div className="relative inline-block">
                        ...
                        {menuOpen === v.id && (
                          <div className="absolute right-0 top-8 z-20 bg-white border border-gray-200 rounded-lg shadow-lg min-w-[120px] py-1">
                            <button
                              onClick={() => { openEdit(v); setMenuOpen(null) }}
                              ...
                              <div className="px-4 py-2 space-y-1">
                                ...
                                <div className="flex gap-2">
                                  <button onClick={() => handleDelete(v.id)}
                                    className="text-xs text-red-600 font-medium px-2 py-1 bg-red-50 rounded">Yes</button>
                                  <button onClick={() => setDeleteConfirm(null)}
                                    className="text-xs text-gray-500 px-2 py-1">No</button>
                                </div>
                              </div>
                            ) : (
                              <button
