...
interface PackageSlide { id: string; image_url: string; caption: string }
interface WhyPackagesConfig { enabled: boolean; title: string; subtitle: string; button_text: string; button_link: string; slides: PackageSlide[] }

const DEFAULT_CONFIG: WhyPackagesConfig = {
  enabled: true, title: 'Why the packages?', subtitle: 'Curated package in detail',
  button_text: 'View All Products', button_link: '/products', slides: [],
}
const EMPTY_SLIDE = (): PackageSlide => ({ id: crypto.randomUUID(), image_url: '', caption: '' })
...
export default function WhyPackages() {
  ...
  useEffect(() => {
    if (raw && !initialized.current) { setConfig({ ...DEFAULT_CONFIG, ...(raw as WhyPackagesConfig) }); initialized.current = true }
  }, [raw])
  ...
  const moveSlide = (idx: number, dir: -1 | 1) => {
    const arr = [...config.slides]; const swap = idx + dir
  ...
  const handleImageUpload = async (id: string, file: File) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `why-packages/${id}-${Date.now()}.${ext}`)
      await uploadBytes(storageRef, file)
      const url = await getDownloadURL(storageRef)
      updateSlide(id, 'image_url', url)
  ...
  const handleSave = async () => {
    ...
    try { await update.mutateAsync({ key: 'why_packages', value: config }); setSaved(true); setTimeout(() => setSaved(false), 2000) }
    ...
    <div className="p-6 space-y-5 max-w-3xl">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {([
            { field: 'title'       as const, label: 'Section Title', placeholder: 'Why the packages?' },
            { field: 'subtitle'    as const, label: 'Subtitle',      placeholder: 'Curated package in detail' },
            { field: 'button_text' as const, label: 'Button Text',   placeholder: 'View All Products' },
            { field: 'button_link' as const, label: 'Button Link',   placeholder: '/products' },
          ...
          <div key={slide.id} className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => moveSlide(idx, -1)} disabled={idx === 0} className="p-1.5 text-gray-400 hover:text-gray-700 border border-gray-200 rounded disabled:opacity-30"><ChevronUp size={13} /></button>
                <button onClick={() => moveSlide(idx, 1)} disabled={idx === config.slides.length - 1} className="p-1.5 text-gray-400 hover:text-gray-700 border border-gray-200 rounded disabled:opacity-30"><ChevronDown size={13} /></button>
                <button onClick={() => setConfig(c => ({ ...c, slides: c.slides.filter(s => s.id !== slide.id) }))}
            ...
            <div>
              ...
              <div className="flex items-start gap-3">
                {slide.image_url
                  ...
                  : <div className="w-24 h-16 rounded-lg bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 text-xs shrink-0">Image</div>}
                <div className="flex-1 space-y-1">
                  <label className="flex items-center gap-2 px-3 py-2 text-xs border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 w-fit">
                    ...
                    {uploading === slide.id ? <Loader2 size={12} className="animate-spin" /> : null} Upload Image
                  </label>
                  ...
                  <input type="text" value={slide.image_url} onChange={e => updateSlide(slide.id, 'image_url', e.target.value)}
            ...
            <div>
              ...
              <input type="text" value={slide.caption} onChange={e => updateSlide(slide.id, 'caption', e.target.value)}
                placeholder="Get your high-quality festival essentials delivered right to your doorstep…"
                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900" />
            </div>
          </div>
        ))}
        <button onClick={() => setConfig(c => ({ ...c, slides: [...c.slides, EMPTY_SLIDE()] }))}
