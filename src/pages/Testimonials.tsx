...
interface Testimonial { id: string; heading: string; review: string; author: string; location: string; image_url: string }
interface TestimonialsConfig { enabled: boolean; title: string; testimonials: Testimonial[] }

const DEFAULT_CONFIG: TestimonialsConfig = { enabled: true, title: 'What Our Customers Say', testimonials: [] }
const EMPTY_TESTIMONIAL = (): Testimonial => ({ id: crypto.randomUUID(), heading: '', review: '', author: '', location: '', image_url: '' })
...
export default function Testimonials() {
  ...
  useEffect(() => {
    if (raw && !initialized.current) { setConfig({ ...DEFAULT_CONFIG, ...(raw as TestimonialsConfig) }); initialized.current = true }
  }, [raw])

  const updateT = (id: string, field: keyof Testimonial, value: string) =>
    setConfig(c => ({ ...c, testimonials: c.testimonials.map(t => t.id === id ? { ...t, [field]: value } : t) }))

  const handleImageUpload = async (id: string, file: File) => {
    setUploading(id); setUploadErrors(e => ({ ...e, [id]: '' }))
    try {
      const ext = file.name.split('.').pop() ?? 'jpg'
      const storageRef = ref(storage, `testimonials/${id}-${Date.now()}.${ext}`)
      await uploadBytes(storageRef, file)
      const url = await getDownloadURL(storageRef)
      updateT(id, 'image_url', url)
    } catch (e) { setUploadErrors(prev => ({ ...prev, [id]: e instanceof Error ? e.message : 'Upload failed' })) }
    finally { setUploading(null) }
  }

  const move = (idx: number, dir: -1 | 1) => {
    const arr = [...config.testimonials]; const swap = idx + dir
  ...
  const handleSave = async () => {
    ...
    try { await update.mutateAsync({ key: 'testimonials', value: config }); setSaved(true); setTimeout(() => setSaved(false), 2000) }
    catch (e) { setSaveError(e instanceof Error ? e.message : 'Save failed') }
    finally { setSaving(false) }
  }

  if (isLoading) return <div className="flex items-center justify-center min-h-96"><Loader2 className="animate-spin text-gray-400" size={24} /></div>

  return (
    <div className="p-6 space-y-5 max-w-3xl">
      <p className="text-xs text-gray-400"><Link to="/site/navbar" className="hover:underline">Website</Link> / Testimonials</p>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><MessageSquare size={20} /> Testimonials</h1>
      ...
      <div className="space-y-4">
          ...
          <div key={t.id} className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => move(idx, -1)} disabled={idx === 0} className="p-1.5 text-gray-400 hover:text-gray-700 border border-gray-200 rounded disabled:opacity-30"><ChevronUp size={13} /></button>
                <button onClick={() => move(idx, 1)} disabled={idx === config.testimonials.length - 1} className="p-1.5 text-gray-400 hover:text-gray-700 border border-gray-200 rounded disabled:opacity-30"><ChevronDown size={13} /></button>
                <button onClick={() => setConfig(c => ({ ...c, testimonials: c.testimonials.filter(x => x.id !== t.id) }))}
                  className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 border border-red-200 rounded ml-1"><Trash2 size={13} /></button>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 space-y-1">
                  <label className="flex items-center gap-2 px-3 py-2 text-xs border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 w-fit">
                    ...
                    {uploading === t.id ? <Loader2 size={12} className="animate-spin" /> : null} Upload Photo
                  </label>
                  {uploadErrors[t.id] && <p className="text-xs text-red-500">{uploadErrors[t.id]}</p>}
                  <p className="text-xs text-gray-400">Or paste URL:</p>
                  <input type="text" value={t.image_url} onChange={e => updateT(t.id, 'image_url', e.target.value)}
                    placeholder="https://…" className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none" />
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {([
                { field: 'heading' as const, label: 'Heading / Title',  placeholder: 'A new way to celebrate!' },
                { field: 'author'  as const, label: 'Author Name',       placeholder: 'Manjunath & Nandini' },
                { field: 'location'as const, label: 'Location',          placeholder: 'Bengaluru' },
              ]).map(({ field, label, placeholder }) => (
                <div key={field}>
                  <label className="block text-xs font-medium text-gray-700 mb-1">{label}</label>
                  <input type="text" value={t[field]} onChange={e => updateT(t.id, field, e.target.value)}
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={t.review} onChange={e => updateT(t.id, 'review', e.target.value)} rows={3}
                  placeholder="Festecart made our Diwali so easy!…"
        ...
        <button onClick={() => setConfig(c => ({ ...c, testimonials: [...c.testimonials, EMPTY_TESTIMONIAL()] }))}
