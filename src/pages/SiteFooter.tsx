...
function LinkEditor({ links, onChange }: { links: FooterLink[]; onChange: (links: FooterLink[]) => void }) {
  ...
  const handleTypeChange = (i: number, typeLabel: string) => {
    ...
    onChange(links.map((l, idx) => idx === i ? { ...l, href: (preset && preset.href !== '') ? preset.href : '' } : l))
  }

  return (
    <div className="space-y-2">
      <div className="grid grid-cols-12 gap-2 px-1 text-xs font-medium text-gray-400 uppercase tracking-wide">
        <div className="col-span-3">Label</div><div className="col-span-4">Type</div>
        <div className="col-span-3">URL</div><div className="col-span-1">On</div><div className="col-span-1" />
      </div>
      {links.map((link, i) => {
        const currentType = getType(link.href); const isCustom = currentType === 'Custom URL'
        return (
          <div key={i} className={`grid grid-cols-12 gap-2 items-center p-3 rounded-lg border ${link.enabled ? 'border-gray-200' : 'border-gray-100 bg-gray-50'}`}>
            ...
            <div className="col-span-4">
              <select value={currentType} onChange={e => handleTypeChange(i, e.target.value)}
                className="w-full px-2 py-1.5 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none">
            ...
            <div className="col-span-1 flex justify-end">
              <button onClick={() => removeLink(i)} className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={13} /></button>
            </div>
          </div>
        )
      })}
      {links.length === 0 && <p className="text-sm text-gray-400 text-center py-3">No links yet</p>}
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        ...
        <textarea value={brand.tagline} onChange={e => setBrand({ tagline: e.target.value })} rows={3}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        <h2 className="font-semibold text-gray-900">"Earn with Festecart" Column</h2>
        <Field label="Column Heading" value={earn.heading} onChange={v => setEarn(e => ({ ...e, heading: v }))} placeholder="Earn with Festecart" />
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setColumns(cols => [...cols, { heading: 'New Column', links: [] }])}
        ...
        {columns.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-4 border border-dashed border-gray-200 rounded-lg">No extra columns — click Add Column to create one</p>
        )}
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <input type="text" value={col.heading}
                    onChange={e => setColumns(cols => cols.map((c, idx) => idx === i ? { ...c, heading: e.target.value } : c))}
                ...
                <button onClick={() => setColumns(cols => cols.filter((_, idx) => idx !== i))}
                  className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded shrink-0"><Trash2 size={14} /></button>
              </div>
              <LinkEditor links={col.links} onChange={ls => setColumns(cols => cols.map((c, idx) => idx === i ? { ...c, links: ls } : c))} />
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select value={bottom.privacy_policy_url} onChange={e => setBottom(b => ({ ...b, privacy_policy_url: e.target.value }))}
          ...
          <div>
            ...
            <select value={bottom.terms_url} onChange={e => setBottom(b => ({ ...b, terms_url: e.target.value }))}
              className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-gray-900">
