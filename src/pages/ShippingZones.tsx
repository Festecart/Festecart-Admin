...
function ZoneMenu({ zone, onClose, anchorRef }: {
  ...
  const left = (rect?.right ?? 0) + window.scrollX - 144 // 144 = menu width
  ...
  return createPortal(
    <div
      ref={menuRef}
      style={{ position: 'absolute', top, left, zIndex: 9999, minWidth: 144 }}
      className="bg-white border border-gray-200 rounded-lg shadow-xl py-1"
    >
      <button
        onClick={() => { navigate(`/shipping-zones/${zone.id}/methods`); onClose() }}
        className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50">
        View Methods
      </button>
      <button
        onClick={() => { navigate(`/shipping-zones/${zone.id}/edit`); onClose() }}
        className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50">
        Edit Zone
      </button>
      <button
        onClick={() => { navigate(`/shipping-zones/${zone.id}/methods/add`); onClose() }}
        className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50">
        Add Method
      </button>
      <button
...
export default function ShippingZones() {
  const navigate = useNavigate()
  const { data: zones = [], isLoading, error } = useShippingZones()
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => navigate('/shipping-zones/vendors')}
            className="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50">
            Courier Vendors
          </button>
          <button
            onClick={() => navigate('/shipping-zones/add')}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        ) : (error as Error)?.message === 'TABLE_MISSING' ? (
          <div className="p-6 space-y-2">
            <p className="text-sm font-semibold text-amber-800">⚠ Database setup required</p>
            <p className="text-xs text-amber-700">
              Run the SQL migration to create the required tables, then refresh.
            </p>
          </div>
        ) : (
          <>
                  ...
                  return (
                    <tr key={zone.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-3">
                        <button
                          onClick={() => navigate(`/shipping-zones/${zone.id}/methods`)}
                      ...
                      <td className="px-5 py-3">
                        {count > 0 ? (
                          ...
                          <button
                            onClick={() => navigate(`/shipping-zones/${zone.id}/methods/add`)}
                      ...
                      <td className="px-5 py-3 text-right">
                        <button
                          ref={el => { btnRefs.current[zone.id] = el }}
                          onClick={e => {
                            e.stopPropagation()
            ...
            {total > PER_PAGE && (
              <div className="px-5 py-3 border-t border-gray-100 text-xs text-gray-400 text-right">
                Currently viewing {(page - 1) * PER_PAGE + 1}–{Math.min(page * PER_PAGE, total)} out of {total}
