...
import {
  db, storage, collection, doc, getDocs, addDoc, updateDoc, deleteDoc,
...
interface Category {
  ...
  icon: string | null; image_url: string | null; display_order: number
...
function CategoryRow({ node, depth, allCategories, onEdit, onDelete, onToggle, onAddChild, deleteId, setDeleteId, deleteMutation }: {
  ...
  onToggle: (c: Category) => void; onAddChild: (parentId: string) => void
  deleteId: string | null; setDeleteId: (id: string | null) => void
    ...
    <>
      <div className="flex items-center justify-between px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0">
        <div className="flex items-center min-w-0" style={{ paddingLeft: `${depth * 28}px` }}>
          {depth > 0 && <span className="text-gray-300 mr-1.5 text-sm select-none">└</span>}
        ...
        <div className="flex items-center gap-2 shrink-0 ml-3">
          <button onClick={() => onToggle(node)} className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-900 whitespace-nowrap">
          ...
          <button onClick={() => onAddChild(node.id)} className="p-1.5 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded" title="Add subcategory"><Plus size={13} /></button>
          <button onClick={() => onEdit(node)} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded"><Pencil size={13} /></button>
          {deleteId === node.id ? (
            <div className="flex items-center gap-1">
              <button onClick={() => onDelete(node.id)} disabled={deleteMutation.isPending}
            ...
            <button onClick={() => setDeleteId(node.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={13} /></button>
          )}
        </div>
      </div>
      {expanded && node.children.map(child => (
        <CategoryRow key={child.id} node={child} depth={depth + 1} allCategories={allCategories}
          ...
          deleteId={deleteId} setDeleteId={setDeleteId} deleteMutation={deleteMutation} />
      ))}
    </>
  )
}
...
export default function Categories() {
  ...
  const uploadBanner = async (file: File) => {
    ...
    } catch (e) { setFormError(`Banner upload failed: ${e instanceof Error ? e.message : String(e)}`) }
  ...
  const toggleActive = async (cat: Category) => {
    await updateDoc(doc(db, 'categories', cat.id), { is_active: !cat.is_active, updated_at: Timestamp.now() })
  ...
  const openAdd = (parentId = '') => {
    setForm({ ...EMPTY, parent_id: parentId }); setFormError(null); setEditId(null); setShowForm(true)
  }
  ...
  const allowedParents = flat.filter(c => !editId || !getDescendantIds(editId).includes(c.id))

  const buildParentOptions = (cats: typeof flat, parentId: string | null = null, depth = 0): { id: string; label: string }[] =>
    cats.filter(c => (c.parent_id ?? null) === parentId && allowedParents.some(a => a.id === c.id))
      .flatMap(c => [
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { label: 'Name *', field: 'name' as const, placeholder: 'Category name' },
              { label: 'Slug',   field: 'slug' as const, placeholder: 'auto-generated' },
              { label: 'Icon',   field: 'icon' as const, placeholder: 'e.g. Flame, Star' },
              { label: 'Description', field: 'description' as const, placeholder: 'Optional' },
              ...
              <div key={field}>
                ...
                <input type="text" value={form[field]}
                  onChange={e => setForm(f => ({
                    ...
                    ...(field === 'name' ? { slug: slugify(e.target.value) } : {}),
                  }))}
            ...
            <div className="flex items-start gap-4">
              {form.image_url ? (
                ...
                <div className="w-32 h-20 rounded-lg border-2 border-dashed border-gray-200 flex flex-col items-center justify-center bg-gray-50 shrink-0">
                  <ImageIcon size={18} className="text-gray-300" /><span className="text-xs text-gray-400 mt-1">No banner</span>
                </div>
              )}
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400 mt-1.5">Recommended: 1200×400px</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button onClick={() => { if (!form.name.trim()) { setFormError('Name is required'); return } saveMutation.mutate() }}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-50">
            ...
            {tree.map(node => (
              <CategoryRow key={node.id} node={node} depth={0} allCategories={flat}
                onEdit={openEdit} onDelete={(id) => deleteMutation.mutate(id)}
                onToggle={toggleActive} onAddChild={(parentId) => openAdd(parentId)}
                deleteId={deleteId} setDeleteId={setDeleteId} deleteMutation={deleteMutation} />
            ))}
          </div>
        )}
